...
class PingPunisher(commands.Cog):
    ...
    async def on_message(self, message):
        if self.modRole in message.content and self.enabled and (not self.isExempt(message.author)):
            targetID = message.author.id  # Yes, message.author.id exists
            for channelid in self.p_channels:
                channel = self.bot.get_channel(channelid)

                # yes, IK + is rendundant, and about the author.mention
                await channel.send("<@" + str(targetID) + ">", delete_after=30)
            ...
            automation_logger.info(
                f"Ping punishment triggered by user {message.author} ({targetID}) in {message.channel.name}")
